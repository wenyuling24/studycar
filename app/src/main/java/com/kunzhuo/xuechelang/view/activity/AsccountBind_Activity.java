 ...
 */
public class AsccountBind_Activity extends BaseActivity {
    ...
    @BindView(R.id.id_addweichatImg)
    CustomImageView idAddweichatImg;
    @BindView(R.id.id_addweichatNick)
    TextView idAddweichatNick;
    @BindView(R.id.id_addWeichatLayout)
    LinearLayout idAddWeichatLayout;
    ...
    private void initViews() {
        context = this;
        idAddWeichatLayout.setVisibility(View.VISIBLE);
        ...
        itemTitlemsg.setText("关联手机号码");
        identifyLoginSubmit.setText("确  认");
        openid = getIntent().getStringExtra("openid");
        nickname = getIntent().getStringExtra("wx_nickname");
        unionid = getIntent().getStringExtra("wx_unionid");
        headimgurl = getIntent().getStringExtra("wx_portrait");

        ImageLoader.getInstance().displayImage(headimgurl, idAddweichatImg);
        idAddweichatNick.setText(nickname);
        ...
        switch (view.getId()) {
            ...
            case R.id.id_identifyLoginBtngetIndentify:
                ...
                if (ToolsUtils.isMobileNO(phone)) {
                    Resquest.sendSendSMS(handler, phone, 4);
            ...
            case R.id.identifyLogin_submit:
                ...
                if (ToolsUtils.isMobileNO(phoneNum)) {
                    if (indentifyCode.length() == 4) {
                        Resquest.addUserWeChat_UaccountAdd(handler2, phoneNum, openid, nickname, headimgurl, unionid, indentifyCode);
