 ...
 * @author zhangQ
 * @date 2016-1-9
 * @description viewpager上面滑动条，去掉了字体样式typeface
 */
...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private int indicatorColor = 0xCC00CCFF;
	private int underlineColor = 0xffD3D3D3;
	private int dividerColor = 0xddaaaaaa;
	...
	// private Typeface tabTypeface = null;//字体样式，没用到--zhangQ
	private int lastScrollX = 0;

	private int tabBackgroundResId = R.drawable.tabs_bg_selector;
	...
	private void updateTabStyles(int position) {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof TextView) {
				...
				tab.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabTextSize);
				if (i == position) {
					tab.setTextColor(0xFFFE4D3D);// 00CCFF
				} else {
	...
	protected void onDraw(Canvas canvas) {
		...
		int x = height - underlineHeight;

		canvas.drawRect(0, x, tabsContainer.getWidth(), height, rectPaint);
		rectPaint.setColor(indicatorColor);
		// 修改TAB标签滑动条高度 height * 17 / 19
		canvas.drawRect(lineLeft, height * 17 / 18/* 19 */, lineRight, height,
				rectPaint);
		// canvas.drawRect(0, height, tabsContainer.getWidth(),
		// 2, rectPaint);
		// draw divider
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageScrolled(int position, float positionOffset,
			...
			setTextColor(0XFF3BBD79, position);// 00CCFF
			invalidate();
	...
	private void setTextColor(int i, int position) {
