...
public class PullDownScrollView extends LinearLayout {
	...
	private String note_release_to_refresh = "松开刷新";
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (ev.getAction() == MotionEvent.ACTION_MOVE) {
			...
			Log.d(TAG, "isElastic:" + isElastic + " canScroll:" + canScroll()
					+ " ev.getY() - preY:" + (ev.getY() - preY));
			if (!isElastic
					&& canScroll()
	...
	private void setMargin(int top) {
		LayoutParams lp = (LayoutParams) mElastic
	...
	public void finishRefresh(String text) {
		if (mElastic == null) {
			Log.d(TAG, "finishRefresh mElastic:" + mElastic);
			return;
		}
		state = IPullDownElastic.DONE;
